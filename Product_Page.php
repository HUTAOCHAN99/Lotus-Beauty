    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        <?php while ($row = $result->fetch_assoc()): ?>
            <div class="bg-white p-4 rounded-lg shadow-md flex flex-col items-center">
                <img src="<?php echo htmlspecialchars($row['image']); ?>" alt="<?php echo htmlspecialchars($row['name']); ?>" class="w-16 h-16 mb-3">
                <a href="Detail_Product.php?product_id=<?php echo $row['product_id']; ?>" class="text-center text-sm font-semibold text-blue-500 hover:underline">
